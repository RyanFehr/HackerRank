/*
         ...
         - Next thing is how do you keep a track of differences. For that I kept map of all the alphabet characters and their occurence of count. 
            For any alphabet, when it occurs in part 1 of string, I increment the count and when it occurs in part 2, I decrement the corresponding count. In the end just add all the differences and divide it
        ...
        var testcaseCount = int.Parse(Console.ReadLine());
            for (var i = 0; i < testcaseCount; i++)
            {
                ...
                else
                {
                    ...
                    var halfLength = inputText.Length / 2;
                    for (var j = 0; j < halfLength; j++)
                    {
                        if (letterMap.ContainsKey(inputText[j]))
                            letterMap[inputText[j]]++;
                        else
                            letterMap.Add(inputText[j], 1);

                        if (letterMap.ContainsKey(inputText[j + halfLength]))
                            letterMap[inputText[j + halfLength]]--;
                        else
                            letterMap.Add(inputText[j + halfLength], -1);
                    }
